...
for (n in n_list) {
    ...
    for (p in p_list) {
        for (seed in seed_list) {
            ...
            #TODO
            # comment outed to calculate only the iteration of DC
            ...
            time_result1 = rbind(time_result1, data.frame(
                ...
                n = n,
                time_dc = time_dc,
                iter_dc = dc_result$dc_iter,
                iter_pdmat = sum(dc_result$make_PDmat_iter)
            ))
        }
        cat("(n, p)=(",n,",",p,") ")
    }
...
for (p in p_list) {
    n = p
    if (METHOD == 'chain') {
        max_num_edge = (p - 1) + (p - 2)
    } else {
        max_num_edge = p*(p-1)/2
    }
    n_nonzero_list = ceiling(quantile(seq_len(max_num_edge), probs=c(0.25, 0.5, 0.75)))
    for (n_nonzero in n_nonzero_list) {
        for (seed in seed_list) {
            data_info = generate_synthetic_data(p=p, n=n, method=METHOD, num_nonzero=n_nonzero, seed=seed)
            K = ceiling(p*(p-1) / 2 / 2)
            tic()
            dc_result = dc_ggm(S=data_info$cov, K=K)
            a = toc(quiet = TRUE)
...
time_result2
#TODO dciterのテーブルを保存＆論文に追記＆先生に相談
...
time_result
time_result = time_result |> group_by(n_p) |> summarise(mean_iter_dc=mean(iter_dc), mean_iter_pdmat=mean(iter_pdmat), p=mean(p)) |> arrange(p)
write_csv(time_result, str_c(path_export, 'table/dc_iter_', METHOD, '_', today_date, '.csv'))
# Visualization ----
